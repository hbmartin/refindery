...
async def test_build_rejects_malformed_converter_output(monkeypatch):
    _install_fake_libs(
        monkeypatch,
        segments=[{"startTime": "not-a-number", "body": "bad"}],
    )
    fetcher = _FetcherStub({"http://t/ep.vtt": "ignored"})
    with pytest.raises(ValidationError):
        await PodcastTranscriptProducer(fetcher=fetcher).build(
            episode_url="http://t/ep",
            transcript_url="http://t/ep.vtt",
            transcript_type="text/vtt",
            chapters_url=None,
