...
class _TranscriptSegment(BaseModel):
    """Validated converter output for one timed transcript segment."""

    model_config = ConfigDict(extra="ignore", strict=True)

    body: str
    start_time: FiniteFloat = Field(default=0.0, alias="startTime", ge=0)


class _TranscriptDocument(BaseModel):
    """Validated podcast-transcript-convert output."""

    model_config = ConfigDict(extra="ignore", strict=True)

    segments: list[_TranscriptSegment] = Field(default_factory=list)


class _ChapterInfo(BaseModel):
    """Validated podcast-chapter-tools output."""

    model_config = ConfigDict(extra="ignore", from_attributes=True)

    start: Annotated[FiniteFloat, Field(ge=0)]
    title: str | None = None
...
def _normalize(pct: object, chapters: object) -> list[_Chapter]:
    ...
    validated = [_ChapterInfo.model_validate(chapter) for chapter in normalized]
    return [(float(chapter.start), chapter.title) for chapter in validated]
...
def _to_segments(*, text: str, mime: str | None) -> list[_Segment]:
    ...
    document = _transcript_document(text=text, mime=mime)
    return [
        (segment.body.strip(), float(segment.start_time))
        for segment in document.segments
        if segment.body.strip()
    ]


def _transcript_document(*, text: str, mime: str | None) -> _TranscriptDocument:
    fmt = _format_from_mime(mime)
    if fmt == "json":
        return _TranscriptDocument.model_validate_json(text)
    ...
    return _TranscriptDocument.model_validate(converters[fmt](text))
